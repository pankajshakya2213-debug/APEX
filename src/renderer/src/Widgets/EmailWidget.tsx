...
import { readEmails, sendEmail, draftEmail } from '@renderer/functions/gmail-manager-api'
...
export default function EmailWidget() {
  ...
  useEffect(() => {
    const handleEvent = (event: any) => {
        const { emails, message, success } = event.detail
        ...
        if (!success && (!emails || emails.length === 0)) {
          setEmails([])
        }
      }
  ...
  const openInbox = async () => {
    ...
    setSelectedEmail(null)
    setComposeStatus('')
    setStatusMessage('Fetching latest mail...')
    setIsLoading(true)
    setEmails([])
    await readEmails(5)
  }
  ...
  const handleSend = async () => {
    if (!composeTo || !composeSubject || !composeBody) {
      setComposeStatus('Fill all fields before sending.')
      return
    }

    setComposeStatus('Sending...')
    const result = await sendEmail(composeTo, composeSubject, composeBody)
    setComposeStatus(result)
  }

  const handleDraft = async () => {
    if (!composeTo || !composeSubject || !composeBody) {
      setComposeStatus('Fill all fields before drafting.')
      return
    }

    setComposeStatus('Saving draft...')
    const result = await draftEmail(composeTo, composeSubject, composeBody)
    setComposeStatus(result)
  }
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold tracking-[0.2em] text-zinc-200">
                {selectedEmail ? 'SECURE MESSAGE VIEW' : isComposeMode ? 'NEW EMAIL' : 'SECURE INBOX LINK'}
              </h2>
              <p className="text-[10px] text-zinc-500 font-mono mt-1 uppercase">
                {selectedEmail
                  ...
                  : `INTERCEPTED BY APEX // ${emails.length} MESSAGES`}
              </p>
            </div>
          </div>
          <button
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        onClick={handleDraft}
                        className="px-5 py-3 rounded-2xl bg-white/5 text-zinc-200 border border-white/10 hover:bg-white/10 transition-all"
                      >
                        Save Draft
                      </button>
                      <button
                        onClick={() => setIsComposeMode(false)}
                        className="px-5 py-3 rounded-2xl bg-red-500/10 text-red-300 border border-red-500/20 hover:bg-red-500/15 transition-all"
                      >
                        Cancel
                      </button>
                    </div>
                    <div className="text-xs text-zinc-400 font-mono">{composeStatus}</div>
                  </div>
                </div>
              </motion.div>
            ) : !selectedEmail ? (
                ...
                {emails.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-zinc-600 gap-4">
                    ...
                    <p className="text-xs tracking-widest opacity-50 font-mono">
                      {isLoading ? 'FETCHING MAIL...' : statusMessage || 'INBOX IS EMPTY'}
                    </p>
                    {!isLoading && statusMessage && (
                      <p className="text-xs text-rose-400 text-center max-w-sm">{statusMessage}</p>
                    )}
                  </div>
                ) : (
                  <div className="flex flex-col gap-4">
