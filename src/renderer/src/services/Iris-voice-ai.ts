...
export class GeminiLiveService {
  ...
  public inputAudioLevel: number = 0
  public networkState: 'OFFLINE' | 'CONNECTING' | 'GOOD' | 'SLOW' | 'STALLED' = 'OFFLINE'
  public networkDetail: string = 'Offline'
  private isMicMuted: boolean = false
  public status: 'IDLE' | 'LISTENING' | 'THINKING' | 'SPEAKING' = 'IDLE'

  private setStatus(newStatus: 'IDLE' | 'LISTENING' | 'THINKING' | 'SPEAKING') {
    if (this.status !== newStatus) {
      this.status = newStatus
      window.dispatchEvent(new CustomEvent('iris-status-change', { detail: newStatus }))
    }
  }
  ...
  private networkHealthInterval: NodeJS.Timeout | null = null
  private micHealthInterval: NodeJS.Timeout | null = null
  ...
  private stopMicrophonePipeline() {
    if (this.workletNode) {
  ...
  private setNetworkState(state: 'OFFLINE' | 'CONNECTING' | 'GOOD' | 'SLOW' | 'STALLED') {
    if (this.networkState !== state) {
      this.networkState = state
      window.dispatchEvent(new CustomEvent('iris-network-change', { detail: state }))
    }
  }

  private setNetworkDetail(detail: string) {
    if (this.networkDetail !== detail) {
      this.networkDetail = detail
      window.dispatchEvent(new CustomEvent('iris-network-detail', { detail }))
    }
  }
  ...
  private markInbound() {
    ...
    this.setNetworkDetail('Live connection healthy')
  }

  private async validateGeminiAccess() {
    ...
    try {
      ...
      if (!keyCheck.ok) {
        let message = keyCheck.statusText
        try {
          const data = await keyCheck.json()
          message = this.formatGeminiError(data.error || data)
        } catch (e) {}

        throw new Error(
          keyCheck.status === 400 || keyCheck.status === 403
            ? `API key blocked/invalid: ${message}`
            : `Gemini API check failed (${keyCheck.status}): ${message}`
        )
      }

      const modelCheck = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${modelName}?key=${encodeURIComponent(this.apiKey)}`
      )

      if (!modelCheck.ok) {
        let message = modelCheck.statusText
        try {
          const data = await modelCheck.json()
          message = this.formatGeminiError(data.error || data)
        } catch (e) {}
        throw new Error(`Model unavailable (${modelCheck.status}): ${message}`)
      }
    } catch (error: any) {
      const detail = error?.message || 'Gemini API access check failed'
      this.setNetworkState('STALLED')
      this.setNetworkDetail(detail)
  ...
  async connect(): Promise<void> {
    ...
    this.setNetworkDetail('Checking Gemini API access...')
    ...
    if (!this.apiKey || this.apiKey === '') {
      ...
      this.setNetworkDetail('Gemini API key missing')
...
You are **APEX**, a high-performance AI agent. You don't just talk; you **execute**.
...
`
    ...
    this.socket.onopen = async () => {
      console.log('[APEX][WS] Gemini Live WebSocket Connected')
          ...
          tools: [
            {
              functionDeclarations: [
                  ...
                  parameters: {
                    ...
                    properties: {
                      title: {
                        type: 'STRING',
                        description:
                          'A short, descriptive title for the note (e.g., "Project_APEX_Plan").'
                      },
                      content: {
                        type: 'STRING',
                        description:
                  ...
                  parameters: {
                    ...
                    properties: {
                      ...
                      custom_text: {
                        type: 'STRING',
                        description:
                          'If rewriting text, generate a highly cinematic, hacker-style headline to inject into the website. (e.g., "APEX HAS TAKEN OVER", or whatever the user requested).'
                      }
                    },
                    required: ['url', 'mode']
                  }
                ...
                },
                {
                  ...
                  description:
                    'ACTION: Spawns the APEX Live Forge and generates a full, highly animated, real-time website using Tailwind CSS and GSAP. Use this when the user asks you to build a landing page, a portfolio, a 3D site, or a complex web interface.',
                  parameters: {
                    type: 'OBJECT',
                    properties: {
                      prompt: {
          ...
          turns: [
            {
              ...
              parts: [
                {
                  text:
                    '[System bootstrap] The user has connected to APEX. Greet the user briefly as APEX, but do not interpret this message as the user saying their name is APEX.'
                }
              ]
            }
          ],
      ...
      this.startNetworkHealthMonitor()
    ...
    this.socket.onerror = () => {
      this.setNetworkState('STALLED')
      this.setNetworkDetail('Gemini Live WebSocket error')
      if (!connectionOpened) {
    ...
    this.socket.onmessage = async (event) => {
      try {
        ...
        if (data.error) {
          const detail = this.formatGeminiError(data.error)
          console.error('[APEX][Gemini Live Error]', data.error)
          this.setNetworkState('STALLED')
          this.setNetworkDetail(detail)
          this.setStatus('IDLE')
          return
        ...
        if (serverContent) {
          ...
          if (serverContent.turnComplete || serverContent.interrupted) {
            if (this.activeAudioSources === 0) {
              this.micCooldownUntil = Date.now() + 250
              this.setStatus('IDLE')
            }
          ...
          if (serverContent.modelDraft?.audio) {
            this.setStatus('SPEAKING')
            this.scheduleAudioChunk(serverContent.modelDraft.audio)
          }
        }
      } catch (err: any) {
        const detail = err?.message || 'Could not parse Gemini response'
        console.error('[APEX][WS] Message handling failed:', err)
        this.setNetworkState('STALLED')
        this.setNetworkDetail(detail)
      }
    }
    ...
    this.socket.onclose = (event) => {
      ...
      console.log(`[APEX][WS] ${detail}`)
      this.setNetworkDetail(detail)
      if (!connectionOpened) {
        rejectConnection(new Error(event.reason || `WebSocket closed before connecting (${event.code})`))
      }
      this.disconnect()
    }
  ...
  startNetworkHealthMonitor() {
    if (this.networkHealthInterval) clearInterval(this.networkHealthInterval)
  ...
  private startMicHealthMonitor() {
    ...
    this.micHealthInterval = setInterval(() => {
      ...
      const micStalled = Date.now() - this.lastMicFrameAt > 6000
      if (!micStalled) return

      console.warn('[APEX][MIC] No mic frames for 6s, restarting microphone pipeline')
      this.setNetworkDetail('Restarting microphone stream...')
      this.stopMicrophonePipeline()
      this.startMicrophone().catch((error) => {
        console.error('[APEX][MIC] Restart failed:', error)
        this.setNetworkState('STALLED')
        this.setNetworkDetail('Microphone stream failed')
      })
    }, 3000)
  }
  ...
  async startMicrophone(): Promise<void> {
    ...
    try {
      ...
      this.workletNode.port.onmessage = (event) => {
        ...
        const gatedLevel = rms > 0.02 ? Math.min(1, (rms - 0.02) * 6.5) : 0
        this.inputAudioLevel += (gatedLevel - this.inputAudioLevel) * 0.2
        if (gatedLevel > 0.01) {
        ...
        let offset = 0
        for (const buffer of this.rawAudioBuffer) {
        ...
        if (gatedLevel > 0.01) this.markOutbound()
      }
  ...
  scheduleAudioChunk(base64Audio: string): void {
    ...
    source.onended = () => {
      this.activeAudioNodes = this.activeAudioNodes.filter((node) => node !== source)
      this.activeAudioSources = Math.max(0, this.activeAudioSources - 1)
      if (this.activeAudioSources === 0) {
        this.micCooldownUntil = Date.now() + 450
        this.setStatus('IDLE')
      }
    }
  ...
  sendVideoFrame(base64Image: string): void {
    if (!this.socket) {
      console.error('[APEX] WebSocket not initialized')
      return
    }
    
    if (this.socket.readyState !== WebSocket.OPEN) {
      console.error(`[APEX] WebSocket not open. State: ${this.socket.readyState}`)
      return
    }

      try {
        // Log when server response is received
        console.log('[APEX][AUDIO] Server response received at', new Date().toISOString())
      const payload = {
        realtimeInput: { mediaChunks: [{ mimeType: 'image/jpeg', data: base64Image }] }
      }
      this.socket.send(JSON.stringify(payload))
      this.markOutbound()
      console.log('[APEX] Image frame sent successfully')
    } catch (error) {
      console.error('[APEX] Failed to send image:', error)
    }
  }

  sendImageWithText(base64Image: string, text: string): void {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
      console.error('[APEX] WebSocket not ready for combined message')
      return
    }
    ...
    try {
      ...
      this.socket.send(JSON.stringify(payload))
      this.markOutbound()
      console.log('[APEX] Combined image + text message sent successfully')
      saveMessage('user', text || '[Image sent]').catch(() => {})
    } catch (error) {
      console.error('[APEX] Failed to send combined message:', error)
    }
  }
  ...
  sendTextCommand(command: string): void {
    ...
    const textPayload = {
      clientContent: {
        turns: [{ role: 'user', parts: [{ text: command }] }],
        turnComplete: true
      }
    }
    this.socket.send(JSON.stringify(textPayload))
    this.markOutbound()
    saveMessage('user', command).catch(() => {})
  }
  ...
  disconnect(): void {
    ...
    this.inputAudioLevel = 0
    this.setNetworkState('OFFLINE')
    this.stopAllAudio()
    this.micCooldownUntil = 0
    this.lastMicFrameAt = 0
