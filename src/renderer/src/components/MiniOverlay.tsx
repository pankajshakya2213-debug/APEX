...
const MiniOverlay = ({
  ...
  return (
    <div className="w-full h-full flex items-center justify-between px-3 bg-zinc-950/90 backdrop-blur-xl rounded-full border border-white/30 drag-region overflow-hidden">
      <div className="flex items-center gap-3 no-drag">
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all duration-300 ${isSystemActive ? (isTalking ? 'border-white/30 bg-white/20 shadow-[0_0_15px_#ffffff]' : 'border-white/50 bg-white/20') : 'border-zinc-700 bg-zinc-900'}`}
        >
          <div
            className={`w-2.5 h-2.5 rounded-full transition-colors duration-300 ${isSystemActive ? (isTalking ? 'bg-white' : 'bg-white') : 'bg-red-900'}`}
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 no-drag">
        <button
          ...
          className={`p-2.5 rounded-full transition-all ml-1 ${!isSystemActive ? 'opacity-30' : isMicMuted ? 'text-red-500 bg-red-500/10' : 'text-white bg-white/10'}`}
        >
        ...
        <button
          ...
          className={`p-3 rounded-full border transition-all duration-500 shadow-lg mx-1 ${isSystemActive ? 'bg-white/20 border-white/30 text-white' : 'bg-zinc-800 border-zinc-600 text-zinc-500 hover:text-red-400'}`}
        >
      ...
      <div className="pl-4 border-l border-white/20 no-drag flex items-center gap-2">
        <button
          onClick={expand}
        ...
        <div className="drag-region cursor-move text-white">
