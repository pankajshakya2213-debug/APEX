  ...
  ipcMain.handle('search-files', async (event, { query, groqKey }) => {
    try {
      ...
      const runNativeCrawler = async () => {
        ...
        if (rawInput) {
          if (os.platform() === 'win32' && (rawInput.length === 1 || rawInput.includes('drive') || rawInput === 'c' || rawInput === 'c:' || rawInput === 'root')) {
            const driveLetter = rawInput.charAt(0).toUpperCase()
            searchRoots.add(driveLetter === 'C' ? 'C:\\' : `${driveLetter}:\\`)
          } else if (['desktop', 'documents', 'downloads', 'music', 'pictures', 'videos', 'home'].includes(rawInput)) {
